"""
Модуль для описания автомата Мура.

Содержит классы перехода и состояния.
"""
...
@dataclass
class MooreTransition[InputType]:
  """
...
@dataclass
class MooreState[InputType, OutputType]:
  """
  ...
  def add_transition(self, transition: MooreTransition[InputType]) -> None:
    """
    Добавляет переход.
  ...
  def replace_transition(self, transition: MooreTransition[InputType]) -> None:
    """
  ...
  def get_available_transitions(
    self, input: InputType, condition_kwargs: Kwargs | None = None
  ) -> list[MooreTransition[InputType]]:
    """
