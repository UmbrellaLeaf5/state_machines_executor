...
class TestMealyMachineAPI:
  ...
  def test_add_transition_entity_from_object(self, empty_machine):
    def cond(input):
      return True
    ...
    empty_machine.add_transition_entity(transition)

    assert empty_machine.get_state_transitions_amount("A") == 1
    assert "A" in empty_machine.get_state_names()
    assert "B" in empty_machine.get_state_names()

  def test_add_transition_entity_from_tuple(self, empty_machine):
    def cond(input):
      return True
  ...
  def test_add_transition_entity_duplicate_no_replace(self, empty_machine):
    def cond(input):
      return True
    ...
    with pytest.raises(ValueError, match="already exists"):
      empty_machine.add_transition_entity(transition, replace=False)

  def test_add_transition_entity_replace(self, empty_machine):
    def cond1(input):
      return True
    ...
    empty_machine.add_transition_entity(transition1)
    empty_machine.add_transition_entity(transition2, replace=True)

    transitions = empty_machine.get_state_transitions("A")
    assert len(transitions) == 1
    assert transitions[0][2] is cond2

  def test_add_transition_entity_invalid_type(self, empty_machine):
    with pytest.raises(TypeError, match=r"Expected .* or tuple"):
      empty_machine.add_transition_entity("not a transition")

  # ----- Новые тесты на update_transitions со смешанными типами -----

  def test_update_transitions_mixed_types(self, empty_machine):
    def cond(input):
      return True
    ...
    empty_machine.update_transitions([transition_tuple, transition_obj])

    assert empty_machine.get_state_transitions_amount("A") == 1
    assert empty_machine.get_state_transitions_amount("B") == 1
    assert "C" in empty_machine.get_state_names()

  def test_update_transitions_with_replace_flag(self, empty_machine):
    def cond1(input):
      return True
    ...
    empty_machine.update_transitions([transition1])
    empty_machine.update_transitions([transition2], replace=True)

    transitions = empty_machine.get_state_transitions("A")
    assert len(transitions) == 1
    assert transitions[0][2] is cond2

  def test_update_transitions_creates_states_automatically(self, empty_machine):
    def cond(input):
      return True
    ...
    empty_machine.update_transitions([("A", "B", cond, func, proc)])
